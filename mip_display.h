...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   mip_display(TTree *tree=0);
   virtual ~mip_display();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     Loop( int first_event=-1, int max_evts = -1, bool arg_verb = true );
...
#ifdef mip_display_cxx
mip_display::mip_display(TTree *tree) : fChain(0) 
{
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      ...
      chain->Add("test-files1a/mip-analysis-ttree*.root/mipanalysis");
...
Int_t mip_display::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t mip_display::LoadTree(Long64_t entry)
{
...
void mip_display::Init(TTree *tree)
{
...
void mip_display::Show(Long64_t entry)
{
...
Int_t mip_display::Cut(Long64_t entry)
{
