...
#include "utils.c"
   ...
   void pf_x2vsx1_sel( int sel=0, const char* infile = "ca4-output/all.root" ) {
      ...
      char hname[1000] ;
      sprintf( hname, "h_log10x2_vs_log10x1_sel%d", sel ) ;
      ...
      char fname[1000] ;
      sprintf( fname, "plots/pf_x2vsx1_sel%d.pdf", sel ) ;
      can -> SaveAs( fname ) ;

    //-------------------


      sprintf( hname, "h_log10x2_vs_log10x1_sel%d_pm", sel ) ;
      TH2F* hp2d_pm = get_hist2d( hname ) ;
      ...
      sprintf( fname, "plots/pf_x2andsx1_sel%d.pdf", sel ) ;
      can2 -> SaveAs( fname ) ;
